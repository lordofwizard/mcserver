...
### No Dedicated IP
Sadly you can't use the host IP into your dedicated IP as it won't let you join that server, you can only join your server by the generated dedicated IP provided by **playit.gg**. If you know how to reverse proxy the tunnel system, you *can* actually use the host IP to make a dedicated IP for your server but I highly doubt that would ever happen (Because there are very few open ports on google cloud and we need special permissions and doing a tcp tunnel from the google cloud shell with their firewall is beyond the scope of this project, also google assigns their cloud shell machines according to the available machines, hence even if we open a port with a tcp connection for a minecraft server it still will be a dynamic ip).
### Mixed Server Specs
